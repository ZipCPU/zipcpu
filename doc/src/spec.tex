...
%% {{{
% Revision History
3.0 & 1/1/2021 & Gisselquist & New debug interface, multi-bus support\\\hline
...
becoming well known among soft-core circles.  So, why build a new processor?
...
contracts.  The ZipCPU has also been a focus for learning about both formal
verification, and how various bus structures work.  For example, starting with
version 3.0, the ZipCPU has full support for both AXI4 and AXI4-lite interfaces.
...
profit: but the talkof the lips tendeth only to penury.'' (Prov 14:23)
\end{preface}
%% }}}
...
The goal of the ZipCPU is to be a simple and very basic soft core CPU.   You
might think of it as a poor man's alternative to the larger architectures
prevalent today.  You might also think of it as an Open Source microcontroller.
...
If/when the ZipCPU supports native floating point, then I anticipate floating
point instructions will also be exceptions to this rule.}  This has resulted in
the choice to drop push and pop instructions, pre-increment and post-decrement
addressing modes, interrupt return instructions and more.
...
\item Wishbone and AXI4 compliant.  All peripherals are accessed just like
...
The ZipCPU also has several memory controllers.  These support Wishbone,
AXI4--lite, and AXI4 bus structures, and may or may not provide a cache
...
\item {\bf\tt OPT\_LOWPOWER}: The ZipCPU has been designed as a low logic
	processor.  Low logic, however, isn't necessarily low power.  If the
...
The next several instructions control the instruction set, and whether or not
...
\item {\bf\tt OPT\_SHIFTS}: If set, the ZipCPU will support shift instructions:
	...
	However, shift register support currently costs a couple hundred LUTs,
	and hence the reason for this option.  A very low logic implementation
...
\item {\bf\tt OPT\_LOCK}: Atomic access on the ZipCPU requires the use of a LOCK
	instruction.  If this value is set, the LOCK instruction will be
...
\begin{itemize} \item {\bf\tt OPT\_START\_HALTED}: The ZipCPU can be configured
	to start up on power up, or it can be configured to wait for a command
	on the debug port before starting.  If {\tt OPT\_START\_HALTED} is
	true, then the ZipCPU will wait to start.

	Incidentally, I almost always leave {\tt OPT\_START\_HALTED} at true.
	This allows me time to load the ZipCPU's program into RAM (or flash)
	externally, and then start the CPU.
...
\item {\bf\tt RESET\_DURATION}: Some architectures require that the CPU stay
	halted for a particular period of time before first attempting to
	access memory or other peripherals.  This behavior can be controlled
	by the {\tt RESET\_DURATION} parameter, controlling how long the CPU
	remains in reset before leaving its initial halted state.
...
\item {\bf\tt OPT\_TRACE\_PORT}: The ZipCPU has a trace port that can be useful
	...
	so it is not enabled by default.  When not enabled, the debug trace port
	will be clamped at zero.

\item {\bf\tt OPT\_CLKGATE}: Some of the ZipCPU's wrappers, notably the AXI4
	and AXI4--lite wrappers, include a clock gating option.  If set to one,
	this option will be enabled and the ZipCPU will halt its clock any
...
\item {\bf\tt OPT\_DMA}:
\item {\bf\tt OPT\_ACCOUNTING}:
\item {\bf\tt EXTERNAL\_INTERRUPTS}:
Controls the number of interrupt wires coming into the ZipSystem CPU wrapper.
This number must be between one and sixteen,
...
%% }}}
\chapter{Current Assessment}\label{chap:assessment}
%% {{{
...
\item The ZipCPU was designed to be a simple and light weight CPU.  It has
	...
	peripherals) has been built that only uses 1.2k~6--LUTs.  When using
	pipelining, the full cache, and all of the peripherals, the ZipSystem
	can take up to 3.9~k LUTs.  Where it fits in between is a function of
	your needs.

	When using an iCE40 FPGA, the minimal ZipBones wrapper plus the
	ZipCPU will fit nicely within the 2.3k~4-LUTs.
...
\item The extremely simplified instruction set of the ZipCPU has proven to be
	...
	only exception being accelerated floating-point support.
...
%% Memory access
\item The ZipCPU can be configured with both instruction and data caches,
	for both Wishbone and AXI.
...
\item The ZipCPU cannot handle addresses larger than 32-bits (4GB).
	This is fundamentally a limit of the 32-bit register architecture
	within the ZipCPU.  While an MMU might help this problem, user
	tasks would remain limited to 4GB of addressable memory.  While
	this may limit the ZipCPU's utility as a generic CPU in the
...
\item When using a DMA to move memory, the only way to tell the CPU that
	...
	clear the cache.  While this isn't ideal, it does work.
